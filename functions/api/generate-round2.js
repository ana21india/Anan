...
export async function onRequestPost({ request, env }) {
  try {
    const { prefA, prefB, likedA = [], likedB = [], seenTitleIds = [] } = await request.json()

    const prompt = buildPrompt(prefA, prefB, 2, likedA, likedB)
    ...
    console.error('generate-round2 error:', err)
